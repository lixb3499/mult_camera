...
def content2detections(content):
    ...
    for i, detection in enumerate(content):
        ...
        # 如果只提供 x 和 y 坐标，假设默认宽度和高度为 (1, 1)。
        if len(detect_xywh) == 2:
            detect_xywh = np.append(detect_xywh, [1, 1])
        # 如果提供 x、y 和cls，假设默认高度为 1。，这行现在实际没用了
        elif len(detect_xywh) == 3:
            detect_xywh = detect_xywh.pop()
            detect_xywh = np.append(detect_xywh, [1])
        ...
        detection_dict = {
            ...
            'licence': data[4],
            'licence_cls': data[5],
            'camera_id': data[6] if len(data) >= 7 else 'None'
        }
...
class Tracker:
    def __init__(self, content, frame_rate=6, tracker_id='scene_1'):
        ...
        for i, detection in enumerate(detections_list):
            self.tracks.append(Tracks(detection['coordinate'], track_id=detection['id'], frame_rate=frame_rate,
                                      licence=detection['licence'], licence_cls=detection['licence_cls'], camera_id=detection['camera_id']))
    ...
    def update(self, content):
        ...
        for detection in detection_list:
            if detection['id'] not in tracker_id_list:
                # self.tracks.append(
                #     Tracks(detection['coordinate'], track_id=detection['id'], frame_rate=self.frame_rate,
                #            licence=detection['licence'], licence_cls=detection['licence_cls']))
                new_track = Tracks(detection['coordinate'], track_id=detection['id'], frame_rate=self.frame_rate,
                                   licence=detection['licence'], licence_cls=detection['licence_cls'], camera_id=detection['camera_id'])
                self.tracks.append(new_track)
                # added_track.append(new_track) #没有confirm，
        return removed_track, added_track
